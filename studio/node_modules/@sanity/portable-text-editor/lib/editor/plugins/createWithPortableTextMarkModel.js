    ...
    return function withPortableTextMarkModel(editor) {
            ...
            if (isSpan) {
                ...
                for (const op of editor.operations) {
                    ...
                    if (op.type === 'merge_node' &&
                        ...
                        const [targetBlock] = slate_1.Editor.node(editor, targetPath);
                    ...
                    if (op.type === 'split_node' &&
                        ...
                        Array.isArray(op.properties.markDefs) &&
                        op.properties.markDefs.length > 0) {
                        const [targetBlock] = slate_1.Editor.node(editor, op.path);
                        ...
                        if (targetBlock &&
                            ...
                            const newMarkDefs = targetBlock.markDefs.filter(def => {
                                return targetBlock.children.find(child => {
                                    return Array.isArray(child.marks) && child.marks.includes(def._key);
                                });
                            });
                            slate_1.Transforms.setNodes(editor, { markDefs: lodash_1.uniq(newMarkDefs) }, { at: op.path, voids: false });
                            editor.onChange();
                        }
                    }
                }
        ...
        editor.apply = op => {
            ...
            if (op.type === 'insert_text') {
                ...
                if (selection &&
                    slate_1.Range.isCollapsed(selection) && ((_a = slate_1.Editor.marks(editor)) === null || _a === void 0 ? void 0 : _a.marks.some(mark => !decorators.includes(mark)))) {
